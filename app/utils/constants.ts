export enum Urls {
    HOME = '/',
    CASES = '/cases',
    DASHBOARD = '/dashboard',
    LOGIN = '/login',
    LOGOUT = '/logout',
    CUSTOMERS = '/customers',
    COMPANIES = '/companies',
    INTERACTIONS = '/interactions'
}

export const PRIMARY_COLOR = `text-primary-500`;
export const ACTIVE_BG_COLOR = `bg-primary-200 hover:bg-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700`;

export const BACKGROUND_COLORS =
    'bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-300';

export const BORDER_COLORS = 'border border-zinc-300 dark:border-zinc-700';

export const BORDER_TOP_COLORS =
    'border-t border-t-zinc-300 dark:border-t-zinc-700';

export const BORDER_RIGHT_COLORS =
    'border-r border-r-zinc-300 dark:border-r-zinc-700';

export const BORDER_BOTTOM_COLORS =
    'border-b border-b-zinc-300 dark:border-b-zinc-700';

export const BORDER_LEFT_COLORS =
    'border-l border-l-zinc-300 dark:border-l-zinc-700';

export const BACKGROUND_GRADIENT = `dark:bg-gradient-to-t dark:from-gray-950`;
